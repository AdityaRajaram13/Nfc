...
const UserProfile = ({ profile, onEdit }) => {
 ...
 useEffect(() => {
  ...
  console.log('Stored UserID:', storedUserID); // Add this line to log the storedUserID
  if (storedUserID) {
  ...
  const qrCodeLink = `${import.meta.env.VITE_FRONTEND_URL}/${userID}`;
  ...
  return (
    <div className="w-96 fixed right-0 h-screen" style={{ backgroundColor: '#111536' }}>
      <div className="flex mt-4 justify-center items-center">
        <button
          className="relative border border-2 border-blue-950 inline-flex items-center justify-center px-6 py-2 overflow-hidden font-mono font-medium tracking-tighter text-white bg-blue-950 rounded-full group"
      ...
      >
        <div className="flex flex-col items-center">
          <div className="relative">
            ...
            {isDropdownOpen && (
              <div
                className="absolute top-12 right-0 mt-20 p-2 bg-blue-950 rounded-lg shadow-md"
                style={{ zIndex: 1 }}
              >
                {/* Include file input inside the dropdown */}
                <input
                  type="file"
                  id="upload-picture"
                  accept="image/*"
